# arduino-esp32 core + the esp32c6 prebuilt SDK (esp32-arduino-libs) merged into
# ONE tree so a single Bazel repo owns both — avoids cross-repo include/-L pain
# (the core sources and the SDK headers/libs/ld must resolve together). The SDK
# lands at sdk/esp32c6/. Bazel package markers are stripped (same as arduino-pico).
pkgs:
let
