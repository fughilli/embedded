...
- **Python deps from PyPI** via `rules_python` (1.5.4 — the 1.x line; 2.x needs
  ...
  `tools/pyocd/requirements.lock`, generated by `compile_pip_requirements`
  (`bazel run //tools/pyocd:requirements.update`). pyocd's native deps
  (capstone, cmsis-pack-manager) all ship aarch64 + x86_64 manylinux wheels, so
  no from-source compiles. Python interpreter is rules_python's hermetic 3.11.
- **libusb from Nix, NOT the wheel.** pyOCD's one native C lib is libusb; the
  ...
  (nixpkgs `libusb1`) + `@hidapi` are materialized like esptool/picotool, and
  the wrapper `py_library //tools/pyocd:nix_libusb` stages the `.so` next to
  `nix_backend.py` (via the `stage_nix_libs` genrule, fixed soname) and points
  pyusb's `find_library` at it. Verified hardware-free:
